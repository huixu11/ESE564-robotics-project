...
\begin{enumerate}
  ...
  \item The experimental grasp extension uses TAMP-style grasp sampling plus RRT-Connect workspace planning for approach and transfer waypoints.
...
\begin{itemize}
  ...
  \item \path{configs/class_panda_grasp.yaml}: experimental TAMP+RRT grasp configuration.
  ...
  \item \path{src/basket_sorting/tamp_grasp.py}: experimental TAMP grasp planner and policy that calls RRT-Connect for approach, transfer, and retreat paths.
  \item \path{src/basket_sorting/rrt.py}: RRT-Connect end-effector workspace planner used by the grasp extension.
...
\newblock In \emph{IEEE International Conference on Robotics and Automation}, 2000.
