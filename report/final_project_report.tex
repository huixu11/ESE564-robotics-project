...
This extension is useful because it makes the system architecture closer to a standard pick-and-place manipulation planner. However, I keep it separate from the main result. The default grasp configuration uses the stable gripper attachment model so that the TAMP skeleton can be evaluated reliably. I also added optional MuJoCo contact-pad infrastructure for a contact-only grasp tool, but that variant still needs tuning because the first pure-contact grasp tests disturbed the object before closing.
...
Experimental TAMP grasp smoke & 5 & 5 & 100\% & 218.60 & 174--240 \\
\bottomrule
\end{tabular}
\end{table}
...
The experimental TAMP grasp extension has its own video artifact at \path{videos/class_panda_grasp_eval_5.mp4}, with the GIF version at \path{videos/class_panda_grasp_eval_5.gif}. Figure~\ref{fig:grasp_tamp} shows still frames from the first grasp episode. This figure is included to document the grasping direction, not to replace the contact-push result as the main physics evidence.
...
\begin{figure}[h]
\centering
\includegraphics[width=0.19\linewidth]{figures/grasp_task_snapshot_0000.png}
\includegraphics[width=0.19\linewidth]{figures/grasp_task_snapshot_0030.png}
\includegraphics[width=0.19\linewidth]{figures/grasp_task_snapshot_0060.png}
\includegraphics[width=0.19\linewidth]{figures/grasp_task_snapshot_0080.png}
\includegraphics[width=0.19\linewidth]{figures/grasp_task_snapshot_0100.png}
\caption{Still frames from the experimental TAMP grasp video. The language caption specifies the command, and the policy executes a sampled grasp skeleton before moving the selected object to the requested basket.}
...
.\.venv\Scripts\python.exe scripts\evaluate.py --config configs/class_panda_grasp.yaml --policy tamp_grasp --episodes 5 --save-video videos/class_panda_grasp_eval_5.gif --video-episodes 5
\end{lstlisting}
