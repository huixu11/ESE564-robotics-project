...
The robot decision-making pipeline has five main components:

\begin{enumerate}
  \item A language parser extracts the target object, target basket, and speed modifier from the instruction.
  \item A color-based perception module estimates the red and yellow object positions from rendered RGB frames.
  \item A scripted pushing FSM converts the parsed task and perceived object position into approach, lower, push, and retreat waypoints.
  \item A differential IK controller converts each Cartesian target into a continuous joint-space update.
  \item The MuJoCo environment integrates object motion during the push phase and checks success after the commanded object enters the goal basket region.
\end{enumerate}
...
python scripts/evaluate.py --config configs/class_panda.yaml --policy push_fsm --instruction "place the cracker box in the left basket" --episodes 3
\end{lstlisting}

Changing only the instruction string changes the selected object and basket:

\begin{lstlisting}[style=cmd]
python scripts/evaluate.py --config configs/class_panda.yaml --policy push_fsm --instruction "place the mustard bottle in the right basket" --episodes 3
\end{lstlisting}
...
\subsection{Perception-driven pushing policy}

The final FSM policy is implemented in \path{src/basket_sorting/push_fsm.py}. Given the parsed language command and the perceived position of the selected object, it computes a push line from behind the object toward the requested basket. The phases are:

\begin{center}
\texttt{approach\_push\_start} $\rightarrow$ \texttt{lower\_to\_push} $\rightarrow$ \texttt{push} $\rightarrow$ \texttt{retreat}.
\end{center}
...
The perception module is implemented in \path{src/basket_sorting/perception.py}. It segments rendered RGB images using color thresholds for the red cracker box and yellow mustard bottle, computes the pixel centroid of each mask, and converts the centroid to an approximate tabletop coordinate with a calibrated affine image-to-world map. The controller uses these perceived object positions instead of directly reading simulator object poses. Simulator poses are still used for randomized reset and final success scoring.
...
\subsection{Physics-based object motion}

The MuJoCo environment contains free joints for the two YCB objects. The earlier prototype used an attachment rule for pick-and-place, but the final evaluation path disables that behavior and uses a pushing proxy during the \texttt{push} phase. The proxy applies planar generalized forces and velocities to the target object's free joint while MuJoCo integrates the object's motion. The baskets are visual goal regions, so their walls do not block the pushed object from entering the target region.

The repository also includes an experimental contact-only configuration in \path{configs/class_panda_contact.yaml}. That path disables the planar proxy, uses simple object collision primitives with visible color markers, localizes objects from an overhead perception camera, disables unrelated Panda link collisions, and moves the target through MuJoCo contact with a dedicated pusher body. The contact FSM uses slow quasi-static pushes, short settle phases, a lift-and-replan step between lateral and forward pushes, and contact-aware pusher offsets so the pusher center does not drive through the desired object center.

\subsection{Experimental TAMP grasp extension}

I also implemented a grasping extension inspired by task and motion planning (TAMP) \cite{garrett2021tamp} and PDDLStream-style sampling of continuous parameters \cite{garrett2020pddlstream}. The implementation is intentionally smaller than a full external TAMP stack such as MoveIt Task Constructor \cite{moveitmtc}, but it follows the same structure: parse the language command into a symbolic goal, sample continuous grasp candidates, check simple feasibility constraints, and execute a pick-and-place skeleton.

The grasp policy is implemented in \path{src/basket_sorting/tamp_grasp.py} and run with \path{configs/class_panda_grasp.yaml}. The symbolic goal has the form \texttt{place(object, basket)}. The planner samples object-specific grasp candidates around the perceived object pose, rejects candidates outside the workspace or beyond the gripper-width limit, and constructs this motion skeleton:

\begin{center}
\texttt{pre\_grasp} $\rightarrow$ \texttt{grasp} $\rightarrow$ \texttt{close} $\rightarrow$ \texttt{lift} $\rightarrow$ \texttt{transfer} $\rightarrow$ \texttt{place} $\rightarrow$ \texttt{release} $\rightarrow$ \texttt{retreat}.
\end{center}

Future versions could replace the hand-written grasp sampler with a learned grasp scorer such as Dex-Net/GQ-CNN \cite{dexnet2} or Contact-GraspNet \cite{contactgraspnet}, while keeping the same symbolic goal and execution skeleton.

This extension is useful because it makes the system architecture closer to a standard pick-and-place manipulation planner. However, I keep it separate from the main pushing result because it was added late in the project. The grasp configuration disables the manual attachment fallback by enabling a dedicated MuJoCo grasp tool. The tool is a regular dynamic body welded to a mocap target, with two side contact pads, a top adhesive contact pad, and a MuJoCo adhesion actuator that turns on during the \texttt{close} and transport phases. Object motion in this path comes from MuJoCo contact and adhesion forces rather than direct object pose setting. The grasp configuration also enables Panda link collision geometry against the YCB objects so that non-gripper arm links cannot pass through objects silently; this follows the planning-scene and allowed-collision-matrix idea used in MoveIt \cite{moveitcollision}. The intentional allowed contacts are the grasp pads with the selected object.
...
The main evaluation used \path{configs/class_panda.yaml}. Each episode samples a random target object, target basket, speed word, and initial object placement. Success is counted when the commanded target object reaches the commanded basket region. The final larger evaluation was run with:

\begin{lstlisting}[style=cmd]
python scripts/evaluate.py --config configs/class_panda.yaml --policy push_fsm --episodes 20
\end{lstlisting}

The contact-rich five-task video was generated with:

\begin{lstlisting}[style=cmd]
python scripts/evaluate.py --config configs/class_panda_contact.yaml --policy push_fsm --episodes 5 --save-video videos/class_panda_contact_eval_5.gif --video-episodes 5
\end{lstlisting}

\subsection{Quantitative results}

Table~\ref{tab:results} reports the final quantitative results. The 20-episode run includes both objects, both baskets, randomized object starts, and speed modifiers.

\begin{table}[h]
\centering
\caption{Class Panda MuJoCo evaluation results.}
\label{tab:results}
...
Experimental contact-rich TAMP grasp & 5 & 5 & 100\% & 254.60 & 192--288 \\
\bottomrule
\end{tabular}
\end{table}

\subsection{Perception sanity check}
...
Figure~\ref{fig:snapshots} shows snapshots from the generated contact-rich five-task pushing video. The sequence illustrates the robot observing two objects, moving through the contact pushing behavior, and the target object entering the requested basket region. The full video file is included at \path{videos/class_panda_contact_eval_5.mp4}, with the GIF version at \path{videos/class_panda_contact_eval_5.gif}.

I also include two instruction-specific videos to make the language interface explicit: \path{videos/push_language_red_to_blue.gif} was generated from ``place the cracker box in the left basket,'' and \path{videos/push_language_yellow_to_green.gif} was generated from ``place the mustard bottle in the right basket.'' In both cases, the GIF frames contain the command as an on-screen language caption, the printed evaluation logs show the same instruction string for each episode, and the robot acts on that specified object-basket pair. Since most PDF viewers do not reliably play animated GIFs, the report includes still frames from these GIFs in Figure~\ref{fig:language_demos}, while the submission zip includes the full animated GIF files.

The experimental TAMP grasp extension has its own video artifact at \path{videos/class_panda_grasp_collision_eval_5.mp4}, with the GIF version at \path{videos/class_panda_grasp_collision_eval_5.gif}. Figure~\ref{fig:grasp_tamp} shows still frames from the first contact-rich, collision-aware grasp episode. This figure is included to document the grasping direction, not to replace the contact-push result as the main physics evidence.

\begin{figure}[h]
\centering
\includegraphics[width=0.19\linewidth]{figures/contact_task_snapshot_0000.png}
\includegraphics[width=0.19\linewidth]{figures/contact_task_snapshot_0080.png}
\includegraphics[width=0.19\linewidth]{figures/contact_task_snapshot_0160.png}
\includegraphics[width=0.19\linewidth]{figures/contact_task_snapshot_0300.png}
\includegraphics[width=0.19\linewidth]{figures/contact_task_snapshot_0420.png}
\caption{Snapshots from the five-task contact-rich MuJoCo pushing video artifact. The camera view shows the language command caption, the two colored YCB objects, the basket regions, and the contact pusher moving objects through MuJoCo contact.}
\label{fig:snapshots}
\end{figure}

\begin{figure}[h]
\centering
\includegraphics[width=0.46\linewidth]{figures/push_language_red_to_blue_snapshot.png}
\includegraphics[width=0.46\linewidth]{figures/push_language_yellow_to_green_snapshot.png}
...
\begin{figure}[h]
\centering
\includegraphics[width=0.19\linewidth]{figures/grasp_collision_cracker_snapshot_0132.png}
\includegraphics[width=0.19\linewidth]{figures/grasp_collision_cracker_snapshot_0165.png}
\includegraphics[width=0.19\linewidth]{figures/grasp_collision_cracker_snapshot_0195.png}
\includegraphics[width=0.19\linewidth]{figures/grasp_collision_cracker_snapshot_0225.png}
\includegraphics[width=0.19\linewidth]{figures/grasp_collision_cracker_snapshot_0255.png}
\caption{Still frames from the experimental contact-rich TAMP grasp video with Panda link/object collision enabled. This sequence shows the cracker-box episode specifically: the visible hand remains above/behind the object while the MuJoCo contact gripper pads grasp and transport the target.}
\label{fig:grasp_tamp}
\end{figure}
...
The main limitation is that perception is a simple calibrated color-segmentation system, so it depends on object colors and camera placement. The stable default pushing dynamics still use a planar MuJoCo force/velocity proxy during the push phase. The new contact-only path is more physically faithful and passes the larger 18/20 randomized gate, but it is much slower and still less reliable than the proxy baseline. The TAMP grasp extension demonstrates a cleaner grasp-and-place decision structure and now uses MuJoCo contact/adhesion rather than a manual attachment rule. It also enables Panda link/object collision checks in the grasp configuration, but it remains a simplified primitive-style collision model and adhesive gripper, not a fully tuned Franka finger contact model with a global sampling-based motion planner. The behavior cloning baseline remains weak because the simple linear policy does not capture the phase-dependent structure of the task. A stronger learned version would use an image encoder, language embeddings, and a recurrent or transformer policy, while retaining the current differential IK controller as a low-level action interface.
...
.\.venv\Scripts\python.exe scripts/evaluate.py --config configs/class_panda_contact.yaml --policy push_fsm --episodes 5 --save-video videos/class_panda_contact_eval_5.gif --video-episodes 5
.\.venv\Scripts\python.exe scripts\evaluate.py --config configs/class_panda.yaml --policy push_fsm --instruction "place the mustard bottle in the right basket" --episodes 3 --save-video videos/push_language_yellow_to_green.gif
.\.venv\Scripts\python.exe scripts\evaluate.py --config configs/class_panda_grasp.yaml --policy tamp_grasp --episodes 5 --save-video videos/class_panda_grasp_collision_eval_5.gif --video-episodes 5
\end{lstlisting}
...
\begin{itemize}
  ...
  \item \path{configs/class_panda_contact.yaml}: experimental contact-only class Panda configuration.
  \item \path{configs/class_panda_grasp.yaml}: experimental TAMP grasp configuration.
  \item \path{assets/class_panda/basket_scene.mjcf}: MuJoCo scene with Panda, YCB objects, baskets, and demo camera.
  \item \path{src/basket_sorting/perception.py}: color-based object perception.
  \item \path{src/basket_sorting/push_fsm.py}: final scripted pushing policy.
  \item \path{src/basket_sorting/tamp_grasp.py}: experimental TAMP grasp planner and policy.
  ...
  \item \path{scripts/perception_sanity.py}: perception localization metric.
\end{itemize}
...
\bibitem{lynch1996stable}
Kevin M. Lynch and Matthew T. Mason.
\newblock Stable pushing: Mechanics, controllability, and planning.
\newblock \emph{The International Journal of Robotics Research}, 1996.

\bibitem{mujocodocs}
Google DeepMind.
\newblock MuJoCo modeling and contact documentation.
\newblock \url{https://mujoco.readthedocs.io/}.

\bibitem{garrett2021tamp}
Caelan Reed Garrett, Rohan Chitnis, Rachel Holladay, Beomjoon Kim, Tom Silver, Leslie Pack Kaelbling, and Tomas Lozano-Perez.
...
\newblock PDDLStream: Integrating symbolic planners and blackbox samplers via optimistic adaptive planning.
\newblock In \emph{International Conference on Automated Planning and Scheduling}, 2020.

\bibitem{moveitmtc}
MoveIt.
\newblock MoveIt Task Constructor pick-and-place tutorial.
\newblock \url{https://moveit.github.io/moveit_task_constructor/tutorials/pick-and-place.html}.

\bibitem{moveitcollision}
MoveIt.
\newblock MoveIt concepts: Planning scene and collision checking.
\newblock \url{https://moveit.ai/documentation/concepts/}.

\bibitem{dexnet2}
Jeffrey Mahler et al.
\newblock Dex-Net 2.0: Deep learning to plan robust grasps with synthetic point clouds and analytic grasp metrics.
\newblock \url{https://bair.berkeley.edu/blog/2017/06/27/dexnet-2.0/}.

\bibitem{contactgraspnet}
Martin Sundermeyer, Arsalan Mousavian, Rudolph Triebel, and Dieter Fox.
\newblock Contact-GraspNet: Efficient 6-DoF grasp generation in cluttered scenes.
\newblock \url{https://research.nvidia.com/publication/2021-03_contact-graspnet-efficient-6-dof-grasp-generation-cluttered-scenes}.
